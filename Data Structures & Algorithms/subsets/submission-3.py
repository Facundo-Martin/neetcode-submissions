class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        ...
        def backtrack(state, start_index):
            # 1. Base Case: Every single node is a valid subset
            results.append(list(state)) 

            # 2. Recursive Step: Loop through ALL remaining choices
            for i in range(start_index, len(nums)):
                state.append(nums[i])   # CHOOSE
