...
# You just perfectly reverse-engineered the dual-identity of every single node in this problem. You hit the nail on the head: every node gets an audition to be the peak of the V, but it also has to act as a stepping stone for the nodes above it.
# The ultimate lightbulb moment: Every single node in the tree is simultaneously evaluated as both the peak of its own V, and as a single leg for its parent's V.

# You've completely cracked the geometry of the problem. That branching restriction is exactly why flattening the tree into an array fails, and why post-order traversal (bottom-up) is the only way to solve it.
